        ...
        <div className="absolute inset-0 bg-black/70 pointer-events-auto" onClick={currentStep.action === 'next' || currentStep.action === 'finish' ? undefined : (e) => e.stopPropagation()}>
          ...
          {currentStep.highlight && (
            <div className={`absolute animate-pulse-ring ${
              currentStep.highlight === 'grid' ? 'top-24 left-52 right-4 bottom-32' :
  ...
  trigger: (variant, time = 0, config = { pitch: 0, chord: null, bend: 0, volume: 100, muted: false, attack: 0, decay: 100, filter: 100, reverb: 0, distortion: 0, pan: 0 }) => {
    ...
    // === SNARE / CLAP ===
      ...
      if (soundType === 'clap') {
        for (let i = 0; i < 3; i++) {
          const delay = i * 0.01;
          const { source, output } = AudioEngine.createFilteredNoise(2000, 1.5, 0.2, t + delay);
          const noiseGain = AudioEngine.ctx.createGain();
          noiseGain.gain.setValueAtTime(0.5, t + delay);
          noiseGain.gain.exponentialRampToValueAtTime(0.01, t + delay + 0.15);
          output.connect(noiseGain);
          noiseGain.connect(volumeGain);
          source.start(t + delay);
          source.stop(t + delay + 0.2);
        }
        ...
        const { source, output } = AudioEngine.createFilteredNoise(4000, 1, 0.25, t);
        const noiseGain = AudioEngine.ctx.createGain();
        noiseGain.gain.setValueAtTime(soundType === 'snareTrap' ? 0.7 : 0.5, t);
    ...
    // === BASS ===
      ...
      if (soundType === 'bassSub') {
        notes.forEach((freq) => {
          ...
          osc2.frequency.value = freq * 2; // Add octave for presence
    ...
    // === SYNTHS ===
      ...
      notes.forEach((freq) => {
        ...
        if (soundType === 'synthPad') {
          ...
          gain.gain.linearRampToValueAtTime(0.2 / notes.length, t + 0.1);
          ...
          gain.gain.setValueAtTime(0.3 / notes.length, t);
          gain.gain.exponentialRampToValueAtTime(0.01, t + 0.2);
...
export default function RhythmRealm() {
  ...
  const TRANSLATIONS = {
    ...
    es: {
      ...
      beatLibraryDesc: '¡Explora ritmos de diferentes géneros!',
      djMode: 'Modo DJ',
      djModeDesc: '¡Modo de actuación en vivo! Mezcla ritmos en tiempo real.',
      accessibility: 'Accesibilidad',
      textToSpeech: 'Texto a Voz',
      textToSpeechDesc: 'Pasa el cursor para escuchar descripciones',
      highContrast: 'Alto Contraste',
      highContrastDesc: 'Aumenta el contraste de colores',
      largeText: 'Texto Grande',
      largeTextDesc: 'Aumenta el tamaño del texto y botones',
      keyboardNav: 'Navegación por Teclado',
      keyboardNavDesc: 'Navegación mejorada con Tab',
      ...
      allEnabled: 'Todas las Funciones Activadas',
    },
    fr: {
    ...
    tl: {
      ...
      createBeat: 'Gumawa ng Iyong Ritmo',
      letsPlay: 'Maglaro Tayo!',
      modes: 'Mga Mode',
      settings: 'Mga Setting',
      freePlay: 'Libreng Paglalaro',
      freePlayDesc: 'Gumawa ng sariling ritmo nang walang limitasyon!',
      tutorial: 'Tutorial',
      tutorialDesc: 'Matuto ng iba\'t ibang istilo at mood!',
      beatLibrary: 'Aklatan ng Ritmo',
      beatLibraryDesc: 'Tuklasin ang mga ritmo mula sa iba\'t ibang genre!',
      djMode: 'DJ Mode',
      djModeDesc: 'Live na performance! I-mix ang mga ritmo sa real-time.',
      accessibility: 'Accessibility',
      textToSpeech: 'Text-to-Speech',
      textToSpeechDesc: 'Mag-hover para marinig ang mga paglalarawan',
      highContrast: 'Mataas na Contrast',
      highContrastDesc: 'Dagdagan ang contrast ng kulay',
      largeText: 'Malaking Text',
      largeTextDesc: 'Palakihin ang text at mga button',
      keyboardNav: 'Keyboard Navigation',
      keyboardNavDesc: 'Pinabuting Tab key navigation',
      voiceControl: 'Voice Control',
      voiceControlDesc: 'Hands-free! Kontrolin ang app gamit ang boses',
      language: 'Wika',
      languageDesc: 'Baguhin ang wika ng app',
      play: 'I-play',
      stop: 'Ihinto',
      tempo: 'Tempo',
      volume: 'Volume',
      ...
      visualThemes: 'Visual Themes',
      about: 'Tungkol Sa',
      enableAll: 'I-enable Lahat ng Accessibility Features',
    ...
    de: {
      ...
      freePlayDesc: 'Erstelle deine eigenen Beats ohne Grenzen!',
      tutorial: 'Tutorial',
      tutorialDesc: 'Lerne verschiedene Stile und Stimmungen!',
    ...
    it: {
      ...
      largeTextDesc: 'Aumenta la dimensione di testo e pulsanti',
      keyboardNav: 'Navigazione Tastiera',
      keyboardNavDesc: 'Navigazione Tab migliorata',
      voiceControl: 'Controllo Vocale',
      voiceControlDesc: 'Mani libere! Controlla l\'app con la voce',
      language: 'Lingua',
  ...
  const toggleCell = (instKey, stepIndex) => {
    ...
    setGrid(prev => {
      ...
      if (newRow[stepIndex]) {
        const variantIndex = instrumentConfig[instKey];
        const sound = SOUND_VARIANTS[instKey][variantIndex];
            ...
            <div className="w-full bg-white/10 rounded-lg p-2 mt-1">
              ...
              <div className="flex gap-0.5 flex-wrap justify-center">
                {currentScenario.tutorial[tutorialStep].targetSteps.map(step => (
                  <div 
                    key={step} 
                    className={`w-5 h-5 sm:w-6 sm:h-6 rounded flex items-center justify-center text-[10px] sm:text-xs font-bold transition-all ${
                      grid[currentScenario.tutorial[tutorialStep].targetInstrument]?.[step] 
         ...
         <div className="flex-1 overflow-x-auto overflow-y-auto p-2 sm:p-4">
            <div className={`${isMobile ? '' : 'min-w-max'} space-y-1 pb-24`}>
                 ...
                 {activeInstrumentIds.map((instKey) => {
                   const variantIdx = instrumentConfig[instKey];
                   const currentVariant = SOUND_VARIANTS[instKey][variantIdx];
